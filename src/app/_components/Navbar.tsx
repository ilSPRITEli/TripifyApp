...
const Navbar = () => {
  ...
  const activeIndex = useMemo(
    () =>
      Math.max(
        0,
        NavItems.findIndex((i) => i.href === current_path)
      ),
    [current_path]
  );
  ...
  return (
    <nav
      className={`flex-row gap-4 fixed bottom-4 w-[90%] mx-auto left-0 right-0 text-secondary flex items-center justify-between z-10`}
    >
      <div
        ref={containerRef}
        className="relative mx-auto bg-primary w-full rounded-full"
      >
        <div className="flex items-center justify-center gap-6 h-16 w-full pb-4 pt-2 text-white">
          {NavItems.map((item, index) => (
            <div
              ...
              className={`flex z-10 ${
                current_path === item.href ? "text-secondary" : "text-white"
              } transition-all duration-150`}
            >
              <Link
                href={item.href}
                className="flex flex-col min-w-12 max-w-12 items-center justify-center text-sm transition-all duration-150"
              >
                <item.icon className="h-6 w-6 mb-1" />
                <span
                  className={`text-xs truncate transition-all duration-300 ease-in-out overflow-hidden ${
